    ...
    <div className="flex flex-row justify-between">
      <nav aria-label="Page navigation w-full]">
        <ul className="inline-flex items-center -space-x-px">
          <li>
            <button onClick={() => handlePageChange(1)} className="block px-3 py-2 ml-0 leading-tight text-gray-500 bg-white border border-gray-300 rounded-l-lg hover:bg-gray-100 hover:text-gray-700">
          ...
          {[...Array(pageCount)].map((_, index) => (
            <li key={index}>
              <button onClick={() => handlePageChange(index + 1)} className={currentPage === index + 1 ? "z-10 px-3 py-2 leading-tight text-white border border-gray-300 hover:bg-gray-800 hover:text-white bg-gray-600" : "px-3 py-2 leading-tight text-gray-600 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 "}>{index + 1}</button>
            </li>
          ))}
          <li>
            <button onClick={() => handlePageChange(pageCount)} className="block px-3 py-2 leading-tight text-gray-500 bg-white border border-gray-300 rounded-r-lg hover:bg-gray-100 hover:text-gray-700">
      ...
      <div>
        <select defaultValue={currentCountPerPage} onChange={(e) => handleCountPerPage(e.target.value)} id="countries" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
