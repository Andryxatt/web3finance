...
export const tokenSlice = createSlice({
  ...
  reducers: {
    ...
    changeTokenByNetwork: (state, action: PayloadAction<any>) => {
      ...
      switch (id) { 
        case 1: {
          state.value = ethereumTokens;
          break;
        }
        case 56: {
          state.value = bscTokens;
          break;
        }
        case 137: {
          state.value = polygonTokens;
          break;
        }
        default: {
          state.value = ethereumTokens;
...
export const { changeTokenByNetwork } = tokenSlice.actions;
// The function below is called a selector and allows us to select a value from
// the state. Selectors can also be defined inline where they're used instead of
// in the slice file. For example: `useSelector((state: RootState) => state.counter.value)`
export const currentTokens = (state: RootState) => state.token.value;
